...
data = [
    go.Scatter(x=new_df['month'],
               y=new_df['actual_max_temp'] / 30,
               mode='markers',
               marker=dict(size=new_df['actual_min_temp']/ 50, color=new_df['actual_min_temp'], showscale=True))
]
layout = go.Layout(title='Temperatures from July 2014 to June 2015', xaxis_title="Month",
                   yaxis_title="Max Temperatures", hovermode='closest')
